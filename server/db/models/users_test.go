...
func testUserToManyUserProviders(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, userProviderDBTypes, false, userProviderColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, userProviderDBTypes, false, userProviderColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	userProvider, err := a.UserProviders().All(tx)
	if err != nil {
		t.Fatal(err)
	}

	bFound, cFound := false, false
	for _, v := range userProvider {
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserProviders(tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserProviders); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserProviders = nil
	if err = a.L.LoadUserProviders(tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserProviders); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	...
	if t.Failed() {
		t.Logf("%#v", userProvider)
	}
}

func testUserToManyAddOpUserProviders(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e UserProvider

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*UserProvider{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, userProviderDBTypes, false, strmangle.SetComplement(userProviderPrimaryKeyColumns, userProviderColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*UserProvider{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserProviders(tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserProviders[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserProviders[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserProviders().Count(tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`CreatedAt`: `timestamp without time zone`, `DisplayName`: `character varying`, `Email`: `character varying`, `ID`: `integer`, `Password`: `character varying`, `Picture`: `text`, `Role`: `enum.user_role('admin','staff','user','disable')`, `TokenVersion`: `bigint`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
