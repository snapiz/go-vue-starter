...
var UserWhere = struct {
	...
	EmailHash:    whereHelperstring{field: `email_hash`},
	...
	Picture:      whereHelpernull_String{field: `picture`},
	TokenVersion: whereHelpernull_Int64{field: `token_version`},
...
type userR struct {
	UserProviders UserProviderSlice
}
...
var (
	userColumns               = []string{"id", "email", "email_hash", "username", "password", "display_name", "picture", "token_version", "role", "state", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"email", "email_hash", "username", "password", "display_name", "picture", "token_version", "updated_at"}
	userColumnsWithDefault    = []string{"id", "role", "state", "created_at"}
...
func (o *User) UserProviders(mods ...qm.QueryMod) userProviderQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"user_providers\".\"user_id\"=?", o.ID),
	)

	query := UserProviders(queryMods...)
	queries.SetFrom(query.Query, "\"user_providers\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"user_providers\".*"})
	}

	return query
}

// LoadUserProviders allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserProviders(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`user_providers`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_providers")
	}

	var resultSlice []*UserProvider
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_providers")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_providers")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_providers")
	}
	...
	if singular {
		object.R.UserProviders = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userProviderR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UserProviders = append(local.R.UserProviders, foreign)
				if foreign.R == nil {
					foreign.R = &userProviderR{}
				}
...
// AddUserProvidersG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserProviders.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddUserProvidersG(insert bool, related ...*UserProvider) error {
	return o.AddUserProviders(boil.GetDB(), insert, related...)
}

// AddUserProviders adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserProviders.
// Sets related.R.User appropriately.
func (o *User) AddUserProviders(exec boil.Executor, insert bool, related ...*UserProvider) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.Provider, rel.ProviderID}
	...
	if o.R == nil {
		o.R = &userR{
			UserProviders: related,
		}
	} else {
		o.R.UserProviders = append(o.R.UserProviders, related...)
	}
